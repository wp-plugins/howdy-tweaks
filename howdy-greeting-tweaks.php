...
class howdy_tweaks {
	...
	function ht_validate( $input ) {
		$original = is_array(get_option('ht_options', array() )) ? get_option('ht_options', array() ) : array();
			...
			if (isset($input['newrow']) && isset($input['0']) ) {
					...
					if (empty($original['0']['label'])) unset($original['0']);
		...
		if (!isset($input['newrow'])) {
			...
			if ($new_row['label']) 	$input = array_merge(array($new_row),$original);
			else 					$input = array_merge(array(),$original);
	...
	function page() {
		...
		$values = get_option( 'ht_options', $defs);
		$greeting = get_option( 'ht_greeting', 'Howdy,');

		echo '<p><label for="greeting">' . __( 'Greeting', 'ht_tweaks' ) . ': <input type="text" name="ht_greeting" id="greeting" value="' . $greeting. '" size="50" /></label></p>';
		?>
		<table class="widefat">
		<thead>
			<tr>
				<th><?php _e( 'Label', 'ht_tweaks' ); ?></th><th><?php _e( 'Favorites', 'ht_tweaks' ); ?></th><th><?php _e( 'Info Links', 'ht_tweaks' ); ?></th><th><?php _e( 'Link', 'ht_tweaks' ); ?></th>
			</tr>
		</thead>
		<tfoot>
			<tr>
		...
		<tbody>
			<tr>
				...
				<td><input type="checkbox" name="ht_options[newrow][favs]" value="" /></td>
		...
		echo '<p>'. __( 'Multisite: Use %ID% as a site ID variable', 'ht_tweaks' ) . '</p>';
		echo '<p>'. __( 'Erase a label and save to remove an entry.', 'ht_tweaks' ) . '</p>';
	...
	function the_info_tweaks( $links ) {
        ...
        foreach($opts as $k => $vals) {
        	if ($vals['info'] != 0) {
        		...
        		if (empty($vals['url'])) $lnk = strip_tags($lnk);
        		$links[] = $lnk; 
        	}
        }        
        /* change "Howdy" */
	...
	function print_it( $data ,$echo = true) { $return  = '<pre>' . print_r( $data, true ) . '</pre>'; if ($echo) echo $return; else return $return; }
}//end class
